	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr {
				...
				assert.Equal(t, task.CreatorID, result.CreatorID)
				assert.Equal(t, task.CreatorDeviceID, result.CreatorDeviceID)
				assert.Equal(t, task.CreatorAddress, result.CreatorAddress)
				mockRepo.AssertExpectations(t)
			}
		})
	}
